...
class _HudScreenState extends State<HudScreen> with WidgetsBindingObserver {
  ...
  Future<void> _initializeHud() async {await SystemChrome.setPreferredOrientations([DeviceOrientation.landscapeLeft,DeviceOrientation.landscapeRight]);if(!mounted)return;final permission=await _gpsService.refreshPermission();if(!mounted)return;if(permission==LocationPermission.denied){final requested=await _gpsService.requestLocationPermission();if(requested==LocationPermission.denied){await _showLocationPermissionDenied();}else if(requested==LocationPermission.deniedForever){await _showLocationPermissionBlocked();}else if(requested==LocationPermission.whileInUse||requested==LocationPermission.always){await _restartGpsAfterPermissionGrant();}}else if(permission==LocationPermission.deniedForever){await _showLocationPermissionBlocked();}if(!mounted)return;await _enterHud();await _startSensors();}
  Future<void> _restartGpsAfterPermissionGrant() async {
    // Let Android finish the permission transition before recreating the
    // native location stream. This avoids the first stream getting stuck
    ...
    if (permission == LocationPermission.whileInUse ||
      ...
      await _gpsService.restart();
    }
  }

  Future<void> _showLocationPermissionDenied() async {await showDialog<void>(context:context,barrierDismissible:false,builder:(dialogContext)=>AlertDialog(title:const Text('LOCATION REQUIRED'),content:const Text('Location permission is required for GPS speed and driving measurements.'),actions:[TextButton(onPressed:() async {final result=await _gpsService.requestLocationPermission();if(!dialogContext.mounted)return;if(result==LocationPermission.deniedForever){Navigator.pop(dialogContext);await _showLocationPermissionBlocked();}else if(result==LocationPermission.whileInUse||result==LocationPermission.always){Navigator.pop(dialogContext);await _restartGpsAfterPermissionGrant();}else {await _gpsService.refreshPermission();}},child:const Text('RE-ALLOW LOCATION')),FilledButton(onPressed:()=>Navigator.pop(dialogContext),child:const Text('CONTINUE'))]));}
