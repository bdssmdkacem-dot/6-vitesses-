...
class OsrmRouteService {
  ...
  void dispose() => _client.close();
    ...
    for (var attempt = 0; attempt < 3; attempt++) {
      try {
        response = await _client.get(uri, headers: const {'Accept': 'application/json'}).timeout(const Duration(seconds: 15));
        if (response.statusCode == 200) break;
        if (response.statusCode < 500) throw Exception('OSRM HTTP ${response.statusCode}');
